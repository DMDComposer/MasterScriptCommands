...
;~ FileMoveDir, % OSC_Unzip, % vvvDir . "\Open Stage Control\",R

Loop, % OSC_Unzip, 1
{
	path := A_LoopFileDir
	DebugWindow(path,Clear:=0,LineBreak:=1,Sleep:=250,AutoHide:=0)
	/*
		Loop %A_LoopFileLongPath%\*, 1
		{
			Loop %A_LoopFileLongPath%\*
				FileMove, %A_LoopFileLongPath%, %path%\%A_LoopFileName% 
		}
	*/
}
