...
; 40, 41, 42, 50, 77, 79
Random, Random_Selection, 40, 42 ;~ Random selection for which songs I want to put on the playlist


;~ m(Page.Evaluate("document.querySelector('#wheelSongId')[2].innerText").value)
oArr := []
;~ m(Page.Evaluate("document.querySelector('#wheelSongId').length").value)
Loop, % Page.Evaluate("document.querySelector('#wheelSongId').length").value
{
	if A_Index-1 == 1
		Continue
	oArr.Push(Page.Evaluate("document.querySelector('#wheelSongId')[" A_Index-1 "].innerText").value)
}
m(oArr)
Return
...
Page.Evaluate("document.querySelector('#wheelSongId').selectedIndex = '" Random_Selection "'") ;~ Must have space between '" xxx "'
