...
DebugWindow(bvar,Clear:=0,LineBreak:=1,Sleep:=250,AutoHide:=0)
...
try ;~ You'd put the try above the line that's triggering the AHK error box, then the catch e { ... } on the lines below your code that triggers the AHK errorbox	
{
	MsgBox, % Page.Evaluate("document.querySelector('.buttonPink').innerText").value = "Spin Wheel" ;~ Place Problem Here
