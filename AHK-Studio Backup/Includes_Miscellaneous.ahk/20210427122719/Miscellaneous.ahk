...
Run_Shortcut_DMD(Program){
	SplitPath, Program, Name, Dir, EXT, NNE, Drive 												; for each item in array, split the path to corresponding variables
	ShortcutDir:="C:\AHK Scripts\_Master Script\Run\Shortcuts"
	Shortcut:= ShortcutDir . "\" . NNE . ".lnk"
	If FileExist(Shortcut){
		DebugWindow("I Do exist",Clear:=0,LineBreak:=1,Sleep:=250,AutoHide:=0)
		Run, % Shortcut
		Return 																			;~ Makes it so if the file exist the script ends here, otherwise it'll continue below
	}
	UI_Path:= "C:\Program Files\AutoHotkey\AutoHotkeyU64_UIA.exe" 
	DebugWindow(Shortcut_UI,Clear:=0,LineBreak:=1,Sleep:=250,AutoHide:=0)
	FileCreateShortcut, % UI_Path, % Shortcut, % Dir, % Chr(34) Program Chr(34)						; SHortcut UI has to go first to tell it to launch UI on to target Shortcut
	DebugWindow(Shortcut,Clear:=0,LineBreak:=1,Sleep:=250,AutoHide:=0)
	While !FileExist(Shortcut) 																;~ Wait til file exists to Run
	{
		DebugWindow("Not here yet...",Clear:=0,LineBreak:=1,Sleep:=250,AutoHide:=0)
	}
	Run, % Shortcut
}
