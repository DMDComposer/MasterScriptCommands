...
if_command_title(command, Dir){
	...
	Loop, read, % Dir
	{
		...
		{
			If A_LoopReadLine contains command =		
			{
				
				Trimmed 	:= StrSplit(A_LoopReadLine,"command = ") 			;~ Find each line with a "command =" in it
				...
				oArray.Push(SubStr(Trimmed.2,1,3))							;~ Show Command 3 Digit Number
				oArray.Push(Substr(Trimmed.2,10))							;~ Only show Title no ";~"
			}
