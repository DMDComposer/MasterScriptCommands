...
Margin 			:= 16
IconSize:=IconW:=IconH:=32

IniRead, Img, C:\AHK Scripts\_Master Script\Resources\Icons.ini, Icons,% Icon,0							;~ Read Ini file of B64 Icons
;~ Images[Icon]:=Img

Img := "C:\AHK Scripts\_Master Script\Resources\Master If Commands Icons\Cogwheel Settings.ico"			;~ Add icon to Main GUI Spawn
...
IE := FixIE(11)
Gui,Add,ActiveX,vwb w%MSC_Height% h%IconSize% hwndhID,mshtml										;~ Have to have some sort of text otherwise it'll error out
FixIE(IE)
...
HTML=
	(
	<body style='margin:0px; background-color:%Background%;color:%Color%;font-size:%FontSize%; font-family:%Font%;'>
		<div style='width:%MSC_Width%px;height:%IconY%px;%Gradient%;'>
			<img align='middle' style='width:%IconW%px;height:%IconW%px;'>
...
Width:= wb.Document.GetElementsByTagName("Div").Item[0].Style.Width := Width+StrLen(Title)+IconW			;~ Grab div > style > width and the width = width set above + 84px
GuiControl, Move, %hID%, % "w" Width "h" IconSize "x" ((MSC_Width-Width)/2)+(IconW/2)					;~ Moving HTML & Centering it Above Edit Field
/*
