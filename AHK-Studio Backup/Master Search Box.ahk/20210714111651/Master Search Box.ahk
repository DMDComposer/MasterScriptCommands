...
gui_change_title(Message,Color := "",Icon := "") {
...
Loop, %search_urls%
{
	Notify().AddWindow("HEERE" query_safe, {Title:""})
	;~ StringReplace, search_final_url, search_url%A_Index%, REPLACEME, %query_safe%
	search_final_url := StrReplace(search_url%A_Index%, "REPLACEME", query_safe)
...
Loop, %command_inputs%
{
	...
	command_final_input := StrReplace(command_input%A_Index%, "REPLACEME", query_safe)
}
command_inputs := 0
Return
