...
{	
	MsgBox, % "You clicked: " event.target.innerText "`n" event.target.style.backgroundColor
	BG:=event.target.style.backgroundColor
	bg = bg.match(/^rgb\((\d+),\s*(\d+),\s*(\d+)\)$/)
	DebugWindow(BG,Clear:=1,LineBreak:=1,Sleep:=500,AutoHide:=0)
}
...
msgbox % "#" + hex(bg[1]) + hex(bg[2]) + hex(bg[3])
