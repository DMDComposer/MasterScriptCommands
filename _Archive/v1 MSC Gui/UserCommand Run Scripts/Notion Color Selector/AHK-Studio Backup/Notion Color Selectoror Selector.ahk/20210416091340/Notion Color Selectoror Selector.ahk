...
;~ #Include ../../Neutron.ahk
#Include ..\..\..\..\..\AHK Studio\Lib\Neutron.ahk

; Create a new NeutronWindow and navigate to our HTML page
neutron := new NeutronWindow()
neutron.Load("Notion Color Selectoror Selector.html")
...
; location!
FileInstall, Notion Color Selectoror Selector.html, Notion Color Selectoror Selector.html
...
Button_Gray(neutron, event)
{
	; event.target will contain the HTML Element that fired the event.
	; Show a message box with its inner text.
	;~ m("You clicked: " event.target.innerText "`n" event.target.style.backgroundColor)	
	Clipboard:=event.target.style.backgroundColor	
	Gosub,NeutronClose
}
Button_Brown(neutron, event)
{	
	Clipboard:=event.target.style.backgroundColor	
	Gosub,NeutronClose
}
Button_Orange(neutron, event)
{	
	Clipboard:=event.target.style.backgroundColor	
	Gosub,NeutronClose
}
Button_Yellow(neutron, event)
{	
	Clipboard:=event.target.style.backgroundColor	
	Gosub,NeutronClose
}
Button_Green(neutron, event)
{	
	Clipboard:=event.target.style.backgroundColor	
	Gosub,NeutronClose
}
Button_Blue(neutron, event)
{	
	Clipboard:=event.target.style.backgroundColor	
	Gosub,NeutronClose
}
Button_Purple(neutron, event)
{	
	Clipboard:=event.target.style.backgroundColor	
	Gosub,NeutronClose
}
Button_Pink(neutron, event)
{	
	Clipboard:=event.target.style.backgroundColor	
	Gosub,NeutronClose
}
Button_Red(neutron, event)
{	
	Clipboard:=event.target.style.backgroundColor	
	Gosub,NeutronClose
}
...
/*
	Button1_MouseMove(neutron, event)
	{
		...
		Button_Color:= event.target.innerText
		DebugWindow(Button Color,Clear:=1,LineBreak:=1,Sleep:=500,AutoHide:=0)
		event.target.innerText := event.target.style.backgroundColor
	}
	
	Button1_MouseLeave(neutron, event)
	{
		; Reset the text of the MouseMove example when the mouse is no longer over
		; it.
		event.target.innerText := "Button"
		;~ DebugWindow(Button_Color,Clear:=1,LineBreak:=1,Sleep:=500,AutoHide:=0)
	}
*/
