...
h := Page.Evaluate("document.querySelector('.buttonSpotify').offsetWidth").value
DebugWindow(h,Clear:=0,LineBreak:=1,Sleep:=250,AutoHide:=0)
...
/*
	...
	While (Page.Evaluate("document.querySelector('.buttonSpotify').offsetWidth").value = 0) AND (Page.Evaluate("document.querySelector('.buttonSpotify').offsetHeight").value = 0) ;if height=0 & Width=0 then not visible
