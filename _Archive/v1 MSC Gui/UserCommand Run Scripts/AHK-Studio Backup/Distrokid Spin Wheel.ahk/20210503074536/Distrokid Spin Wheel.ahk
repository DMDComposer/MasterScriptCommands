...
m("Spin Wheel is visible")
m("Pausing before first spin hit")
;~ Click on Spin note once, not twice, but THRICE!
...
m("Second spin is visible")
m("Pausing before second spin hit")
Page.Evaluate("document.querySelector('.buttonPink').click()")
Loop {
	Sleep, 10
}Until Page.Evaluate("document.querySelector('.buttonPink').value = 'Spin once more!'")
m("Third spin is visible")
m("Pausing before third spin hit")
Page.Evaluate("document.querySelector('.buttonPink').click()")
Loop {
	Sleep, 10
}Until Page.Evaluate("document.querySelector('.gameOver > div:nth-child(1) > div:nth-child(4) > a:nth-child(1)').innerText = 'Check out the playlist'") ;~ Look for Spotify Check out the Playlist to appear
m("congrats message is visible")
