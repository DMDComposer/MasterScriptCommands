...
Loop, % Page.Evaluate("document.querySelector('#wheelSongId').length").value
{
	if A_Index-1 == 0
		Continue
	oRnd.Push(Page.Evaluate("document.querySelector('#wheelSongId')[" A_Index-1 "].innerText").value)
}
m(oRnd)
Random, Random_Selection, 1, oRnd.length() ;~ Random selection for which songs I want to put on the playlist
m(oRnd[Random_Selection])
/*
	for Key, Value in oRnd
	{
		MsgBox, % oRnd[Random_Selection].Key " " oRnd[Random_Selection].Value
	}
*/
Return
