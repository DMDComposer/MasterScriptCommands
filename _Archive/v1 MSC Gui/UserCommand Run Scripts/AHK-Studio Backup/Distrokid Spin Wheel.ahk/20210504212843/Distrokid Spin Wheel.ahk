...
;~ var := Page.Evaluate("document.querySelector('.buttonSpotify').innerText").value = "Connect with Spotify"
...
While (var.offsetWidth=0) AND (var.offsetHeight=0) ;if height=0 & Width=0 then not visible
...
DebugWindow(var.value,Clear:=0,LineBreak:=1,Sleep:=250,AutoHide:=0)
